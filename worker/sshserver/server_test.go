...
func (s *sshServerSuite) TestSSHPublicKeyHandler(c *gc.C) {
	...
	mockAuthorizer.EXPECT().AuthorizedKeysPerUser(gomock.Any()).DoAndReturn(func(userTag names.UserTag) ([]string, error) {
		if userTag.Name() == "alice" {
			return []string{string(s.userSigner.PublicKey().Marshal())}, nil
		}
	...
	tests := []struct {
		...
		{
			name:          "valid user and invalid public key",
			user:          names.NewUserTag("alice"),
			key:           notValidSigner,
			expectedError: true,
		},
