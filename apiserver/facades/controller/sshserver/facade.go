...
func (f *Facade) PublicKeyAuthentication(userTag names.UserTag, publicKey ssh.PublicKey) (bool, error) {
	modelUUIDs, err := f.systemState.ModelUUIDsForUser(userTag)
	if err != nil {
		return false, errors.Errorf("failed to get model uuids for user: %v", err)
	}
	for _, uuid := range modelUUIDs {
		authKeys, err := f.authorizedKeysPerModel(uuid)
		if err != nil {
			return false, errors.Errorf("failed to get authorized key for model: %v", err)
		}
	...
	return false, errors.Errorf("failed to find a matching public key")
}
